...
- Why: Clicking a user avatar anywhere (messages, member lists) should open a right sidebar — similar to Slack's profile panel — showing the user's avatar, display name, email, role, presence status, and local time.
- Evidence:
...
- Why: Each message currently shows a persistent reply button underneath it, adding visual clutter. Slack-style behavior shows reply (and other actions) only in a hover popup bar at the top-right of the message — the same bar where the emoji reaction picker already lives.
- Add:
...
- Impact: Cleaner message list UI that matches user expectations from Slack.
- Effort: Low.
...
- Why: Long message histories have no visual separation between days, making it hard to orient in time. Slack shows horizontal lines with a date label between messages from different days.
- Add:
...
## 36. Redesign Landing (`/`) and Workspace Creation Flow
